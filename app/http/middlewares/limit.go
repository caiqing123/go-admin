...
//
func LimitIP(limiters *limiterlib.Limiter) gin.HandlerFunc {
	return func(c *gin.Context) {
		// 针对 IP 限流
		key := limiter.GetKeyIP(c)
		if ok := limitHandler(c, key, limiters); !ok {
			return
		}
		c.Next()
	}
...
func LimitPerRoute(limiters *limiterlib.Limiter) gin.HandlerFunc {
	return func(c *gin.Context) {
		// 针对 IP + 路由进行限流
...
func limitHandler(c *gin.Context, key string, limiters *limiterlib.Limiter) bool {

	// 获取超额的情况
	rate, err := limiters.Get(c, key)
	if err != nil {
		logger.LogIf(err)
		response.Abort500(c)
		return false
