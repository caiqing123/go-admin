...
// ValidateInAdminArray 自定义规则，验证数组是否存在
func ValidateInAdminArray(Ids []int, id int, errs map[string][]string) map[string][]string {
	if collection.Collect(Ids).Contains(id) {
		errs["ids"] = append(errs["ids"], "不能删除管理员")
	}
	return errs
}
