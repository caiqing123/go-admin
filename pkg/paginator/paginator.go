...
//			   "*",
//         )
func Paginate(c *gin.Context, db *gorm.DB, data interface{}, baseURL string, perPage int, Filed string) Paging {
...
func (p *Paginator) initProperties(perPage int, baseURL string, tableName string) {
	...
	p.Sort = p.ctx.DefaultQuery(config.Get("paging.url_query_sort"), tableName+".created_at")
...
func (p *Paginator) getTotalCount() int64 {
	var count int64
	if err := p.query.Preload(clause.Associations).Count(&count).Error; err != nil {
		return 0
	}
	return count
}
